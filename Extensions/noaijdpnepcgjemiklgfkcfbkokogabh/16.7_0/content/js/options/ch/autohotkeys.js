...
function INIT(){
  ...
  if(line.length>1) SL_ACTIVE_TAB = line[1];
	...
	GEBI("SRV8").value=FExtension.store.get("SL_HK_opt");
  	...
  	var tempHK12 = FExtension.store.get("SL_HK_bb2box");
  	if(tempHK12=="true"){
	...
	GEBI("SRV12").value=FExtension.store.get("SL_HK_bb2").replace("Escape","Esc");
 ALL_NONES();
 Resolve_the_HK_conflicts();
}
 ...
 if(UNBLOCK == 0) {
  ...
  if (result==0){
	  GEBI("HK_conflicts_msg").innerText="";
		...
		FExtension.store.set("SL_HK_bb2", GEBI('SRV12').value.replace("Esc","Escape"));
		...
		if(GEBI("IMT_HK5").checked==false) window['parent'].GEBI("SL_HIDE5").style.display="block";
	        ...
	        window['parent'].GEBI('SRV6').value=GEBI('SRV12').value;
		window['parent'].GEBI("SL_translation_mos_bbl2").checked=GEBI("IMT_HK12").checked;
		if(GEBI("IMT_HK12").checked==false) window['parent'].GEBI("SL_HIDE6").style.display="block";
