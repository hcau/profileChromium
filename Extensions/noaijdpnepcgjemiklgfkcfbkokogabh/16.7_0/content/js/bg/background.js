...
//		ImTranslatorBG.setDefault();				
		if(FExtension.store.get("SL_Version") != "15000.80000"){	
...
//		      		        if(nmr > 1) ImTranslatorBG.DO_CNTR("2121",SL_langSrc+"/"+SL_langDst,text.length);
//					else ImTranslatorBG.DO_CNTR("2131",SL_langSrc+"/"+SL_langDst,text.length);
...
//			if(TR=="IT" && SL_langDst!="") dir=SL_langDst;
	...
	SL_GOOGLE_DETECT: function(baseUrl,SL_Params){
		var METHOD = "POST";
		baseUrl = baseUrl +"?"+ SL_Params;
	    	if(FExtension.store.get('SL_GAPI1')=="0"){
			if(SL_Params!=""){
        	           var arr = SL_Params.split("q=");
	                   var arr_1 = arr[1].split("&");
			   var TXT = arr_1[0];
			}
			var tk = ImTranslatorBG.GetHash(TXT,ImTranslatorBG.BaseGTK);
			baseUrl = "https://translate.googleapis.com/translate_a/t?anno=3&client=te&v=1.0&format=text&tk="+tk;
		}	
			...
			var timeStampInMs = ImTranslatorBG.LB();
		  	if(FExtension.store.get('SL_GAPI1')=="1" && FExtension.store.get('SL_GAPI2')=="1"){
				ImTranslatorBG.BUBBLE_DET="<#>";
				ImTranslatorBG.INTERNAL_ONLY=1;
				ImTranslatorBG.TRIGGER=0;
				return false;
		  	}
		  	var ATTEMPT = 0;

			if(FExtension.store.get('SL_GAPI1')=="1" && FExtension.store.get('SL_GAPI2')=="0"){
				baseUrl = baseUrl +"?"+SL_Params;
				METHOD = "GET";
                        	ATTEMPT=1;
		  	}
			...
			ajaxRequest.onreadystatechange = function(){
				...
				if(ajaxRequest.status == 429) {
					if(FExtension.store.get('SL_GAPI1')=="0"){
						FExtension.store.set('SL_GAPI1',1);
						FExtension.store.set('SL_GAPI1_ts',timeStampInMs);
						ajaxRequest=null;
						ImTranslatorBG.SL_GOOGLE_DETECT(baseUrl, SL_Params);
					}
					if(ATTEMPT==1){
						FExtension.store.set('SL_GAPI2',1);
						FExtension.store.set('SL_GAPI2_ts',timeStampInMs);
						ajaxRequest=null;
						ImTranslatorBG.BUBBLE_DET="<#>";
						ImTranslatorBG.TRIGGER=0;
						return false;
					...
					if(ajaxRequest.readyState == 4 && ajaxRequest.status == 200){
						...
						if(resp.indexOf('"src":"')!=-1){
							...
							ImTranslatorBG.BUBBLE_DET=resp.src;
						} else {
							if(resp.indexOf('","')!=-1){
								var TMP = resp.split('","');
								var TMP2 = TMP[1].split('"');
								ImTranslatorBG.BUBBLE_DET=TMP2[0];
		  	...
		  	ajaxRequest.open(METHOD, baseUrl);
		  	ajaxRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		  	ajaxRequest.send(SL_Params);
	},
	...
	SL_GOOGLE: function(baseUrl, SL_Params, theQ){
		var METHOD = "POST";
	    	if(FExtension.store.get('SL_GAPI1')=="0"){
			if(SL_Params!=""){
        	           var arr1 = SL_Params.split("sl=");
	                   var arr1_1 = arr1[1].split("&");
			   var SRL = arr1_1[0];
        	           var arr2 = SL_Params.split("tl=");
	                   var arr2_1 = arr2[1].split("&");
			   var TRL = arr2_1[0];
        	           var arr3 = SL_Params.split("q=");
	                   var arr3_1 = arr3[1].split("&");
			   var TXT = arr3_1[0];
			}
			var tk = ImTranslatorBG.GetHash(TXT,ImTranslatorBG.BaseGTK);
			baseUrl = "https://translate.googleapis.com/translate_a/t?anno=3&client=te&v=1.0&format=text&sl="+SRL+"&tl="+TRL+"&tk="+tk;
		}	
		  ...
		  ajaxRequest.onreadystatechange = function(){
			...
			if(ajaxRequest.status == 429) {
				if(FExtension.store.get('SL_GAPI1')=="0"){
					...
					FExtension.store.set('SL_GAPI1_ts',timeStampInMs);
					ajaxRequest=null;
					ImTranslatorBG.SL_GOOGLE(baseUrl, SL_Params, theQ);
				}
				if(ATTEMPT==1){
					FExtension.store.set('SL_GAPI2',1);
					FExtension.store.set('SL_GAPI2_ts',timeStampInMs);
	...
	LB: function(){
		  ...
		  var timeStampInMs_old1 = FExtension.store.get('SL_GAPI1_ts');
		  var difference1 = Math.ceil((timeStampInMs - timeStampInMs_old1) / 60000);

		  if(difference1>=ImTranslatorBG.GFROZEN) {
			FExtension.store.set('SL_GAPI1',0);
			FExtension.store.set('SL_GAPI1_ts',0);
		  }

		  var timeStampInMs_old2 = FExtension.store.get('SL_GAPI2_ts');
		  var difference2 = Math.ceil((timeStampInMs - timeStampInMs_old2) / 60000);
		  if(difference2>=ImTranslatorBG.GFROZEN) {
			FExtension.store.set('SL_GAPI2',0);
			FExtension.store.set('SL_GAPI2_ts',0);
		  }
