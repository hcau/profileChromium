...
setTimeout(function(){
	window.onresize = function() { 	window.resizeTo(495,GEBI('SL_body').clientHeight+80); }
},500);
	 ...
	 if(f=="") f=mySourceLang;
	 	...
	 	if(PROV.toLowerCase()=="google"){
			text = text.trim();
  			...
  			if(nmr > 1) CNTRP('2321',f+"/"+t, text.length);
			else CNTRP('2331',f+"/"+t, text.length);
	 	}


		var baseUrl = ImTranslator_theurl+"dotrans.php";
		var cgi = "dir=c"+f+"/"+t+"&provider="+PROV.toLowerCase()+"&text="+encodeURIComponent(text);
...
//			if(ajaxRequest.readyState == 4 && ajaxRequest.status == 200){
			if(ajaxRequest.readyState == 4){
		             ...
		             if(resp.indexOf('>Url Too Long<')!=-1 || resp.indexOf('>Request URL Too Long<')!=-1 || resp.indexOf('"ArgumentOutOfRangeException')!=-1) resp=PROV.replace('google','Google') + ": "+ FExtension.element(FExtension.store.get("SL_LOCALIZATION"),'extADVstu').replace("XXX","4000");
		             if(resp.indexOf('<#<')!=-1 || resp.indexOf('&lt;#')!=-1) resp=PROV.replace('google','Google') + ": "+ FExtension.element(FExtension.store.get("SL_LOCALIZATION"),"extnotrsrv");
			     if(resp=="" || resp.indexOf("<h1>Not Found</h1>")>-1) resp=PROV.replace('google','Google') + ": "+ FExtension.element(FExtension.store.get("SL_LOCALIZATION"),"extnotrsrv");
  ...
  if(!document.getElementById("PL_lbframe")){
    ...
    document.getElementById('SL_player3').appendChild(die);
		  ...
		  if(frame)	frame.parentNode.removeChild(frame);
		     ...
		     audioElement.setAttribute('style', 'width:470px;margin-top:-15px;margin-left:-24px;');
...
function DODetection(myTransText) {
  ...
  if(myTransText!=""){
    ...
    var SL_Params="client=dict-chrome-ex&sl=auto&tl=en&q="+encodeURIComponent(newTEXT) + "&tbb=1&ie=UTF-8&oe=UTF-8";    
	 ...
	 if(text!=""){
		...
		ajaxRequest.onreadystatechange = function(){
			...
			if(ajaxRequest.readyState == 4){
		             ...
		             if(resp.indexOf('>Url Too Long<')!=-1 || resp.indexOf('>Request URL Too Long<')!=-1 || resp.indexOf('"ArgumentOutOfRangeException')!=-1) resp=PROV + ": "+ FExtension.element(FExtension.store.get("SL_LOCALIZATION"),'extADVstu').replace("XXX","4000");
		             if(resp.indexOf('<#<')!=-1 || resp.indexOf('&lt;#')!=-1) resp=PROV + ": "+ FExtension.element(FExtension.store.get("SL_LOCALIZATION"),"extnotrsrv");
			     if(resp=="" || resp.indexOf("<h1>Not Found</h1>")>-1) resp=PROV + ": "+ FExtension.element(FExtension.store.get("SL_LOCALIZATION"),"extnotrsrv");
